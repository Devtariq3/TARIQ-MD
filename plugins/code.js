...
async (conn, mek, m, { q, reply }) => {
    try {
        ...
        if (!q || !/^\d{8,15}$/.test(q)) {
            return await reply("❌ Invalid phone number. Example: `.pair 237695704816`");
        }

        // Fetch pairing code from API
        const response = await fetch(`https://Devtariq3-web.onrender.com/pair/code?number=${q}`);
